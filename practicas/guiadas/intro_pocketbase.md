...
PocketBase es un **Backend As A Service** ligero pero razonablemente potente y que podemos instalar en nuestra propia máquina de manera sencilla, ya que es simplemente un ejecutable de unos 12 Mb. Tiene una serie de funcionalidades que nos permiten desarrollar el *backend* de manera mucho más rápida que si lo tuviéramos que hacer desde cero:

- **Gestión de usuarios y autenticación**: CRUD de usuarios de la aplicación, login y logout. La autenticación se puede hacer con usuario/contraseña o con servicios de terceros (Google. Github, ...) a través de OAuth2.
- **Base de datos realtime**: tiene embebida una instancia de SQLite y proporciona un API de mayor nivel de abstracción que SQL para poder trabajar con los datos. Además podemos observar los cambios en los datos en tiempo real.
...
La interfaz web está disponible por defecto en `http://localhost:8090/_/`). Desde ella podemos añadir usuarios a nuestra aplicación y asignarles mecanismos de autenticación, crear las colecciones (==tablas) de la base de datos, etc.
...
Vamos a crear una aplicación para gestionar la lista de la compra, en la que cada usuario tendrá almacenada la lista de cosas que quiere comprar. Cada ítem a comprar tendrá simplemente un *nombre* y un campo booleano *comprado* que indicará si ya lo hemos comprado o no. De momento para simplificar cada usuario solo puede tener una lista.
...
3. Pulsa el botón `New Record` en la parte superior derecha de la pantalla 
...
PocketBase ofrece una API con diferentes implementaciones: REST, Javascript y Go. Desde Javascript, que es el lenguaje que usamos en la asignatura, podríamos acceder tanto al API REST como al nativo Javascript. Usaremos este último, de estilo RPC, ya que es el más directo desde JS.
...
### Autentificarse y leer registros
...
try {
    ...
    for(const item of lista) {
        console.log(item.nombre, item.comprado ? "comprado" : "pendiente")
    }
...
onRecordCreateRequest((e) => {
    //si es superusuario, que ponga el id que quiera
    if (e.hasSuperuserAuth()) return
    //ponemos como usuario el autentificado actualmente
    e.record.set("usuario", e.auth.id)
    //dejamos que la creación del registro siga su curso. Si no llamamos a next no se realizará
    e.next()
}, "lista")
```

El código anterior lo tendríamos que poner en una nueva carpeta `pb_hooks` dentro del directorio donde tengamos PocketBase. El archivo donde lo coloquemos debe tener la extensión `pb.js`. En la documentación a este archivo siempre lo llaman `main.pb.js`. En Windows seguramente haga falta parar y rearrancar PocketBase, en Linux/Mac no debería hacer falta.

Nótese que en el código de `crear.js` ya no es necesario fijar el valor del campo `usuario` dentro de la constante `datos`, salvo que seamos superusuarios el *hook* lo sobreescribirá siempre.
