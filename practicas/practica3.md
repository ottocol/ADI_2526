...
El objetivo de esta práctica es desarrollar una web que sirva de cliente al backend de la práctica 1. En la implementación debéis usar un *framework JS*. En principio debe ser Vue pero podéis usar cualquier otro como Angular, Svelte, React, SolidJS,... Eso sí, si usáis uno distinto a Vue tendréis que encontrar las equivalencias de los requisitos que se piden aquí para vuestro framework (por ejemplo el router de Vue tiene un equivalente en el resto de frameworks, que normalmente también se llama Router).
...
Si usáis otro framework tendréis que buscar el equivalente de Pinia para vuestro caso. Por ejemplo en SolidJS y Svelte se llama `Store`.
...
- Hasta *1 punto*: implementar el listado y eliminación de items con otro framework cualquiera (Svelte, Angular, React,...). No es necesario implementar editar y ver detalles en este caso. Tendréis que hacer un proyecto nuevo ya que la estructura del proyecto de un *framework* no os va a servir para otro.
